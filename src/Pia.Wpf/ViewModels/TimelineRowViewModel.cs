...
/// <summary>Read-only row for one recorded tool decision. Everything here is metadata — the store holds no tool
/// arguments, no results and no paths. A property that named a file or carried a payload would fail the
/// reflection assert in <c>RunProgressViewModelTimelineTests</c>.</summary>
public sealed class TimelineRowViewModel : ObservableObject
{
    ...
    /// <summary>"Step N" while the row's step is still in the projected plan; null when it is not — a replan
    ...
    /// <summary>Set on the first row BELOW the exception block, which draws the rule separating the two halves —
    /// cheaper than a separator item, which the trace's row-shape guard would have to allow for.</summary>
