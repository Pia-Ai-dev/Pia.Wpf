...
public class AssistantChatService : IAssistantChatService, IDisposable
{
    ...
    public async Task<IReadOnlyList<AssistantChatMessageHit>> SearchMessagesAsync(
        Guid chatId, string term, int limit = 20, CancellationToken ct = default)
    {
        // A blank term must find nothing: '%%' matches every row, which would turn a recovery lookup into a
        ...
        try
        {
            ...
            using var command = Connection().CreateCommand();
            // ESCAPE, because a term can legitimately contain % or _ — an error string or a file path does.
            command.CommandText = """
                SELECT Id, Ordinal, Role, Content
                FROM AssistantChatMessages
                WHERE ChatId = @ChatId AND Content LIKE @Pattern ESCAPE '\'
            ...
            var hits = new List<AssistantChatMessageHit>();
            using var reader = await command.ExecuteReaderAsync(ct);
            while (await reader.ReadAsync(ct))
            {
                hits.Add(new AssistantChatMessageHit(
                    Guid.TryParse(reader.GetString(0), out var id) ? id : Guid.Empty,
                    reader.GetInt32(1),
                    reader.GetString(2),
                    Snippet(reader.GetString(3), term)));
            }
    ...
    /// <summary>A window around the match rather than the head of the message: a hit whose snippet does not
    /// contain the term is useless for deciding whether to read the whole row.</summary>
    private static string Snippet(string content, string term, int radius = 120)
    {
        var at = content.IndexOf(term, StringComparison.OrdinalIgnoreCase);
        if (at < 0)
            return content.Length <= radius * 2 ? content : content[..(radius * 2)] + "…";

        var start = Math.Max(0, at - radius);
        var end = Math.Min(content.Length, at + term.Length + radius);
        var window = content[start..end];
        return (start > 0 ? "…" : string.Empty) + window + (end < content.Length ? "…" : string.Empty);
    }
