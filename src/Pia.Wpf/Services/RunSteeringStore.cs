...
/// <summary>
/// Default <see cref="IRunSteeringStore"/>. Two lock-free <see cref="ConcurrentDictionary{TKey,TValue}"/>
/// maps keyed on the run id — the sink of the dispatch running it here, and the pending pause intent — so
/// nothing on the UI thread ever waits on the run pool (the <c>ExecutingRunStore</c> discipline). Nothing
/// here throws.
/// <para>
/// TWO maps rather than one composite entry, deliberately: the sink is written and removed by the DISPATCH
