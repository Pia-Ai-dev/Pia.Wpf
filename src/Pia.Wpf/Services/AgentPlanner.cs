...
/// rather than a degenerate 1-step Planned run (§16 R10). A plan turn may instead decline the goal via
/// <c>emit_plan</c>'s <c>cannotGround</c>/<c>question</c> members, returning <c>PlanResult.Decline</c> instead of
/// the R10 degrade and skipping the firm retry, since a decline is not silence. Provider usage is summed off the drained
...
public sealed class AgentPlanner : IAgentPlanner
{
    ...
    /// <summary><c>emit_plan</c> for a plan turn, including the decline (<c>cannotGround</c>/<c>question</c>) members.</summary>
    ...
    /// <summary>
    /// <c>emit_plan</c> for a replan turn: no decline members, since a replan already has enough context that
    /// declining doesn't apply. Same tool name as <see cref="EmitPlanTool"/> on purpose — the two are never sent
    /// on the same turn.
    /// </summary>
    ...
    /// <summary>
    /// The <c>emit_plan</c> argument schema. <paramref name="steps"/> defaults to <c>null!</c> rather than a
    /// nullable <c>PlanStepArg[]?</c>, because <c>AIFunctionFactory</c> propagates a nullable default into the
    /// array's item schema — which would let a step's title/intent go null too, and let a plan straight past
    /// <see cref="ValidatePlan"/>'s rejection of that.
    /// </summary>
    ...
    /// <summary>What one plan turn said, plus what it cost.</summary>
    ...
    {
        // Folds in any answered clarification, so a resumed plan doesn't re-ask a question the user already
        // answered. Returns `goal` unchanged when nothing is recorded, which is every fresh launch.
        ...
        // emit_plan), so it skips the firm retry, and it wins over any steps that came with it.
    ...
    /// <summary>The decline outcome. The question is model-generated text derived from the user's goal, so it is
    /// logged only via <see cref="LoggingExtensions.SensitiveDebug"/>; <c>LogInformation</c> records only that a
    /// decline happened and whether it came worded.</summary>
    ...
    public async Task<PlanResult> ReplanAsync(RunContext ctx, string? failure, Persona persona, AiProvider provider, CancellationToken ct)
    {
        ...
        // A replan deliberately does not offer the decline: it already has a goal the model planned once, plus
        // the completed steps and failure detail, so a model that invents cannotGround anyway is just read as a
        // no-steps turn (the code below reads `turn.Steps` only) and hits the ordinary firm retry then degrade.
    ...
    /// <see cref="PlanTurn.Steps"/> null and <see cref="PlanTurn.CannotGround"/> false — distinct from a decline.
    /// </summary>
    /// <param name="tool">The <c>emit_plan</c> variant this turn offers — <see cref="EmitPlanTool"/> on a plan
    /// turn, <see cref="EmitRevisedPlanTool"/> on a replan turn.</param>
        ...
        ToolCallHandler toolHandler = (call, _) =>
        {
            if (string.Equals(call.Name, "emit_plan", StringComparison.Ordinal))
            {
                try
                {
                    ...
                    // Blank normalizes to null so a nullness check downstream can answer "was a question worded?".
