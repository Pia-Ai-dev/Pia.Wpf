...
| Routines editor | `Routines_Field_Name`, `_Goal`, `_Kind`, `_Recurrence`, `_DayOfWeek`, `_Month`, `_DayOfMonth`, `_Time`, `_Date`, `_Provider`, `_Persona`, `_Effort`, `_Quiet`, plus `Routines_Save` / `Routines_Cancel`. A routine started from a blueprint also renders its fill-in slots, per slot: `Routines_Slot_<name>`, keyed on the slot's `Name`, not its visible `Label`. The AI assist above the fields is `Routines_Draft_Description` plus `Routines_Draft_Generate`; `Routines_RequiredHint` is the line that shows while `Routines_Save` is disabled. The detail pane's `Routines_Detail_Tools` reads back what the selected routine may use |
| Routines editor, meeting kind | Setting `Routines_Field_Kind` to the Teams-meeting row swaps the form: `Routines_Field_MeetingUrl` and `Routines_Field_MeetingConsent` appear, and `_Goal` / `_Provider` / `_Persona` / `_Effort` / the tool picker collapse. They stay in the logical tree while collapsed, so a prefix match still finds them — assert on visibility, not on presence. Saving refuses without a Teams link AND a ticked consent box |
