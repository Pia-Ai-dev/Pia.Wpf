...
| Advanced Creation overlay | Opened by `Routines_Draft_Advanced`, `PersonaEdit_Advanced` or `TemplateEdit_Advanced`, all three beside the one-shot Sparkle button they do not replace. Inside: `AdvancedCreation_Opening` / `AdvancedCreation_Start` (the first turn only), then per question `AdvancedCreation_Q_<questionId>` — the id is the model's own key, so match the prefix rather than a fixed name — plus `AdvancedCreation_Opt_<optionLabel>` for a multi-choice box, and `AdvancedCreation_Skip` / `AdvancedCreation_Send`. `AdvancedCreation_Summary` reads back what the model has understood, `AdvancedCreation_Thinking` is present only while a turn is in flight. `AdvancedCreation_Error` plus `AdvancedCreation_RetryButton` / `_DismissError` is the inline failure banner; Escape after the first answer raises `AdvancedCreation_DiscardPrompt` with `AdvancedCreation_ConfirmDiscard` / `_KeepEditing`. It is an `OverlayDialogPanel`, so "Use this draft" and Cancel are the shared `PART_PrimaryButton` / `PART_CloseButton` below — the primary is disabled until `AdvancedCreation_DoneMessage` appears. |
