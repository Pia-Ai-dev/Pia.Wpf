...
| Screen capture picker (`ScreenCapturePickerView` inside `ScreenCapturePickerContentDialog`) | `ScreenCapturePicker_Refresh`; per-target `ScreenCapturePicker_Target_<key>`, keyed on `ScreenCaptureTargetViewModel.AutomationKey` — the window handle for a window, `DISPLAYn` for a display, because every monitor reports handle 0 and the handle alone would not be unique. The rows are `RadioButton`s whose UIA name is the window title; Capture and Cancel are `ContentDialog`'s own buttons and carry no id. Test lock: `ViewAutomationIdTests`. |
