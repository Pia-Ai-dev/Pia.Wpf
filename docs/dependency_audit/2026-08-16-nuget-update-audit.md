...
| 12 | **Microsoft.ML.OnnxRuntime** · **org.k2fsa.sherpa.onnx** | ~~1.24.4 → 1.29.0 · 1.12.40 → 1.13.5~~ | **Done 2026-08-30, and this verdict was wrong twice over.** They are not two rows — both packages ship `runtimes/win-x64/native/onnxruntime.dll`, so the ORT pin must track what sherpa bundles (1.27.1, *not* 1.29.0). And there is a functional gain: Windows wide-char file paths, which the model cache needs. Landed at 1.13.5 / 1.27.1. See [../speech_to_text/2026-08-30-stt-stack-update-check.md](../speech_to_text/2026-08-30-stt-stack-update-check.md). |
...
| 13 | ~~Microsoft.ML.OnnxRuntime 1.29.0~~ | **Resolved 2026-08-30 at 1.27.1, and the risk framing was right.** "A green build proves nothing" is exactly why it was verified by loading every real model out of `%LOCALAPPDATA%PiaModels` and decoding a German clip, not by building. 1.29.0 was rejected: it would outrank sherpa's bundled runtime in the DLL collision. |
| 14 | ~~org.k2fsa.sherpa.onnx 1.13.5~~ | **Resolved 2026-08-30.** "Verifiable only in a live transcription run" turned out to be too pessimistic — a throwaway console probe over the cached models settled it: Parakeet and the speaker embedding decode bit-identically, Whisper decodes *differently* (better on tiny, worse on medium, one clip). The model-format worry was the right one to have; it is Whisper decoding, not model acceptance, that moved. |
