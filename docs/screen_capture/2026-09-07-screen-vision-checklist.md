...
**G3 answered 2026-09-09: yes** — on a deployment with an image-capable endpoint the provider
accepts the interleaved user image message and the model attends to it; it quoted a nonce sentence
that existed nowhere but the picture. D2 and D2a are both proven live. The first attempt failed at
the server's model router for want of a vision endpoint, which is a different question and is
logged as the endpoint-capability gap below. See
...
- [x] **D2. Inject the capture into the round loop. This is G3.** Drain the round's parked captures
