...
```bash
dotnet test -- --filter-method "*AParkedRunsFlowBodyNamesWhyItParked*"
...
```csharp
    /// <summary>A resume that claims the row and then dies before dispatch must re-park with the reason the
    /// next resume needs — <c>needs-goal</c>/<c>needs-input</c>/<c>plan-approval</c> preserve their own token
    /// (losing it would silently drop re-planning, answer persistence, or the approval card), other reasons
    /// get the generic <c>resume-interrupted</c> diagnostic.</summary>
    [Theory]
...
No `Designer.cs` regeneration needed (per Task 3.1 Step 3's finding — runtime lookups never go through it). Re-run the parity test:
...
public async Task RejectPlan_CallsResumeServiceRejectPlanAsync()
{
    ...
    await _runs.PauseAsync(run.Id, AgentRunOrchestrator.PlanApprovalReason, TestContext.Current.CancellationToken);
    await vm.RefreshAsync();
    Assert.True(vm.RejectPlanCommand.CanExecute(null)); // exercises the [NotifyCanExecuteChangedFor] wiring
...
None of the three tests above actually discriminate Step 6's `[NotifyPropertyChangedFor(nameof(ShowNudgeBox))]` fix — `ShowNudgeBox`'s getter returns the right value with or without that attribute, since a getter's return doesn't depend on whether `PropertyChanged` fired. Add a test that subscribes BEFORE pausing and asserts the notification itself, so removing the attribute in a future edit would fail this test:
...
dotnet test -- --filter-method "*RejectPlan_CallsResumeServiceRejectPlanAsync*"
...
Deliberately NO `.ConfigureAwait(false)` on this OUTER `await` (unlike the inner one) — the continuation below (the `foreach` over `_allSessions` and its `SetForeignRunActive`/`SetPlanApprovalParkActive` calls) must resume on the UI thread `_syncContext.Post` dispatched onto, matching this file's own stated rule (`RestoreActiveRunAsync`'s doc comment: "UI-affine on purpose (no `ConfigureAwait(false)`)") and its own warning that `_allSessions`/`_ownRunIds` are plain, non-thread-safe collections mutated from the UI thread elsewhere. A trailing `ConfigureAwait(false)` here would resume the loop on a pool thread instead and race those collections — exactly the hazard class this file's existing comments call unacceptable.
...
No `Designer.cs` regeneration needed (per Task 3.1 Step 3's finding). Run the parity test:
...
**A FIFTH site, easy to miss**: `Dispose()` has its OWN separate unsubscribe block (`AssistantViewModel.cs:1901-1909`), distinct from the reattach-time unsubscribe inside `AttachToActiveSession` — `ForeignRunActiveChanged` is unsubscribed in BOTH places, not just one. Current:
