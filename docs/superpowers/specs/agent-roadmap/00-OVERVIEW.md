...
**AMENDED 2026-08-04: "There is no Rank 2" is spent — [Batch 18](18-goal-grounding-and-clarification.md) enters
at Rank 2.** Left standing per this file's habit; read the Rank column, not the sentence. Two things this
insert deliberately does **not** do. It does not reorder Rank 1 — 18's own §9 says it *lengthens* that round
(a real provider declining to plan is its central premise and nothing else tests it), so promoting it above the
smoke round would have widened the gap it sits behind; the owner ranked it here for that reason. And it carries
no Phase and no Size, because it has not been built and this table's sizes elsewhere were written after the
fact. **What put it on the list was a single observed run**, not an audit: a goal of `ggg` produced a
four-step plan that executed to completion while the model was on record asking what the goal meant. Its file
records three independent losses of that signal, one shipped invariant it cannot be built without breaking
(`AgentRunOrchestrator.cs:186`, "a resume must NOT re-plan" — **08 D1**, not to be conflated with 18 D1), and
**two findings that shrank the batch rather than growing it**: `emit_step_result` already tells the model in its
own description that prose is not a report, so a third step outcome would **not** have fixed the repro and the
owner declined to add one (**18 D6**); and the Flow item's standing "no user content" rule
(`AgentRunNotificationSurface.cs:186`) forbids putting the model's question on the card at all, which settles
where the two surfaces of **18 D5** divide instead of leaving it to be designed. The same file's three publish
filters (`:157`/`:170`/`:176`) also mean **"parked ⇒ card" is false on two of 18's own paths** — a delegated
child publishes none, and neither does the chat the user is actively watching.
...
### Opened by Batch 18 (2026-08-05) — known, reasoned, not closed

Four items, enumerated **by name** per the Rank-1 row's convention, carried over from
[`18-goal-grounding-and-clarification.md`](18-goal-grounding-and-clarification.md) §9. **Nothing comes off the
list here** — this batch is the third consecutive change to move the number **up**. Each says why a unit fact
cannot stand in for the round.

1. **A real provider declining to plan.** Whether a real model, handed a decline channel, actually *uses* it on
   thin input — or still fabricates four steps — is **the batch's central premise**, and a stubbed planner
   cannot settle it. Every unit fact in G2 asserts what happens *given* a decline; none can establish that a
   decline ever arrives. This is the sharpest item, because if the answer is "it still fabricates", the batch's
   plan-time half closes nothing and only the local pre-flight (G1) survives. §2 is the reason to be
   pessimistic: `emit_step_result`'s description already told the model, in plain words, that prose is not a
   report — and the model ignored it. A tool description is a request.
2. **The parked card and the question, rendered.** Whether a user reading the card understands what is being
   asked and *where to answer it*. §4.3 makes this sharper than the tool-approval card that item 2 of "Opened
   by the hermes §7 sweep" already books: **no `FlowAction` carries input and no card renders a text box**, so
   by rule the card cannot show the question and the answer is typed somewhere other than the card. A test can
   assert the card's `Title`/`Body` are token-keyed and question-free — §8.6, and it does — but not that the
   round trip is *discoverable*. A card that says a run is waiting without conveying where to answer leaves the
   run stuck, which is the same failure class as that hermes item, reached from a different direction.
3. **DE/FR render of the new copy.** Five new keys × three files, parity-enforced by the suite and register-
   checked by hand against their neighbouring families. **Parity is not rendering** — the standing reason this
   line appears in every one of these sections.
4. **Whether a real run that parks mid-plan is recoverable by a real user.** `18 D4` permits a run to park for
   a question **any number of times**, a risk the owner was shown and accepted (spec §5). What repeated parking
   *feels* like — and whether a user can still get such a run to finish — is exactly what no unit test
   observes. `MidPlanAskTests.ARunMayParkToAskMoreThanOnce_ThereIsNoCap` pins that the second park is *possible*;
   it cannot pin that it is *tolerable*.

**One thing this section must not be read as covering.** Everything above is manual work *in addition to* a
green suite — and, unlike when this section was first written, that suite has now actually run. **Gate closed
2026-08-05 at `9de00a49`: `3620 / 0 failed / 1 skipped`, +148 over the pre-batch baseline at `8fecbc85`**, the
nine never-executed classes named in the batch spec all green in isolation (`94 / 0 failed`), and one
unrelated pre-existing flake (`AssistantChatConcurrencyTests`) ruled out by a 4/4 isolated re-run — see the
Rank-1 row above for the full account. **None of that shortens this section.** The four items above were never
claims about unit coverage; they are exactly what a green suite cannot observe, and they stay open regardless.
