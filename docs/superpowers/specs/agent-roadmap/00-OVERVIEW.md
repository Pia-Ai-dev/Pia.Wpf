...
green suite. This batch's suite has **never been executed on any platform** — it was built on macOS, where
`net10.0-windows` tests cannot run. Nine new test classes and thirteen extended ones await their first
Windows/CI run, which is a **gate**, not a smoke item, and it comes first. The batch spec names the four
highest-risk classes to watch.
