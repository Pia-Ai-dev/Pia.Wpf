...
public class AssistantViewParseTests
{
    ...
    /// </summary>
    [Fact]
    public void RunProgressPanel_RendersATimelineRow_WithItsStepOutcomeAndDecision()
    {
        var (texts, emptyLineVisibility) = WpfStaHost.Run(() =>
        {
            var vm = CreateRunProgressViewModel();
            var panel = new RunProgressPanel { DataContext = vm };
            ...
            // AFTER the pump, deliberately: the expand's own (store-less) load posts its projection through the
            // dispatcher and would otherwise overwrite this. This is what the real load does once it has rows.
            vm.HasNoTimeline = false;

            // The trace's ItemsControl is declared markup, so it IS in the logical tree; its generated
            // containers are not (the walk here is logical — the same documented limit that keeps the message
            // template out of reach). So instantiate the row template the way WPF does and bind one row to it:
            // that is the TEMPLATE INSTANTIATION the builder's open item said no test could reach, and it pins
            // every path in the template without a layout pass on a view whose Loaded handlers are a hazard.
