...
public sealed class StepPersonaResolverTests
{
    ...
    [InlineData(true)]
    [InlineData(false)]
    public async Task AStepPersona_IsComposedWithTheRunsOwnUnattendedShape(bool unattended)
    {
        // Read off the run default rather than passed in, so a step persona cannot quietly drop the framing —
        // or gain it on an interactive Planned run, where a question does reach somebody.
        ...
        var runDefault = new StepPersonaSetup(
            Persona("Pia"), Provider("run-provider"),
            new AssistantTurnSetup("run system", null, false, false, Unattended: unattended));

        await Build().ResolveAsync(analyst.Id, runDefault, tokenizationEnabled: false, Ct);

        _composer.Received(1).PrepareTurn(analyst, Arg.Any<AiProvider>(), Arg.Any<IReadOnlyList<AtCommand>>(),
            Arg.Any<bool>(), Arg.Any<bool>(), Arg.Any<string?>(), unattended: unattended);
    }
    ...
    public async Task PrepareTurnThrows_FallsBackToTheRunDefault()
    {
        ...
        _providers.GetDefaultProviderForModeAsync(Arg.Any<WindowMode>()).Returns(Provider());
        _composer.PrepareTurn(Arg.Any<Persona>(), Arg.Any<AiProvider>(), Arg.Any<IReadOnlyList<AtCommand>>(),
            Arg.Any<bool>(), Arg.Any<bool>(), Arg.Any<string?>(), unattended: Arg.Any<bool>()).Throws(new InvalidOperationException("bad persona prompt"));
        var runDefault = RunDefault();
    ...
    public async Task SuggestAgentModeIsNeverOfferedInsideARun()
    {
        ...
        _composer.DidNotReceive().PrepareTurn(Arg.Any<Persona>(), Arg.Any<AiProvider>(),
            Arg.Any<IReadOnlyList<AtCommand>>(), Arg.Any<bool>(), suggestAgentModeEligible: true, environmentRoot: Arg.Any<string?>(), unattended: Arg.Any<bool>());
        _composer.Received(1).PrepareTurn(analyst, Arg.Any<AiProvider>(), Arg.Any<IReadOnlyList<AtCommand>>(),
            tokenizationEnabled: true, suggestAgentModeEligible: false, environmentRoot: Arg.Any<string?>(), unattended: Arg.Any<bool>());
    }
